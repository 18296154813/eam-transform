    ...
    <div>
                    ...
                    <div class="ibox tt-from-table">
                        <div class="ibox-content">
                            <form role="form" class="form-inline">
                                ...
                                <tt-simple-tree-root-v2 label="类型" v-model="conditions.assetsTypeId"
                                                        :data="tree.assetType"
                                                        :option="{key:'id',value:'name'}"></tt-simple-tree-root-v2>
                                <tt-simple-tree-root-v2 label="网点" v-model="conditions.pointId"
                                                        :data="tree.point"
                                                        :option="{key:'id',value:'name'}"></tt-simple-tree-root-v2>
                                <!--<tt-simple-select label="网点" v-model="conditions.pointId" :data="Map.point"-->
                                <!--show-undefined></tt-simple-select>-->

                                <div class="btn-toolbar pull-right" role="toolbar">
                                    <div class="btn-group">
                                        <button @click="showUpdateModal(3,tableSelectData[0])" v-if="hasOneChecked"
                                                v-shiro:permission="'asset:updateStatus'"
                                                class="btn btn-outline btn-primary" type="button">
                                            {{tableSelectData[0].status === 3 ? '完成' : '维修'}}
                                        </button>
                                        <button @click="showUpdateModal(4,tableSelectData[0])" v-if="hasOneChecked"
                                                v-shiro:permission="'asset:updateStatus'"
                                                class="btn btn-outline btn-primary" type="button">
                                            {{tableSelectData[0].status === 4 ? '撤回报废' : '报废'}}
                                        </button>
                                        <button @click="openStockTake()" v-shiro:permission="'asset:stockTake:add'"
                                                class="btn btn-outline btn-primary" type="button">开启盘点
                                        </button>
                                        <button @click="deleteAll()" v-if="hasChecked"
                                                v-shiro:permission="'asset:delete'" class="btn btn-outline btn-danger"
                                                type="button">删除
                                        </button>
                                    </div>
                                    <div class="btn-group">
                            ...
                            <div class="table-responsive">
                                <tt-table v-bind:data="tableData" :selection="true" v-model="tableSelectData">
                                    <template slot="tt-body-operation" scope="props">
                                        <button @click="showQRCodeModal(props.row)"
                                                class="btn btn-table btn-primary btn-rounded" type="button">QRCode
                                        </button>
                                        <button @click="showOperationRecordModal(props.row)"
                                                v-shiro:permission="'asset:record:getByUuid'"
                                                class="btn btn-table btn-primary btn-rounded" type="button">操作记录
                                        </button>
                                    </template>
                                </tt-table>
                            </div>
                            <!-- 分页 -->
                            <tt-pagination :count="pagination.count" @listener="getTablePaginationList" button-size="sm"
                                           class="pull-right"></tt-pagination>
                            <div class="clearfix"></div>
            ...
            <form role="form" class="validation">
                <div class="row">
                    <div class="col-sm-12"><!--<div class="col-sm-6 b-r">-->
                        ...
                        <tt-simple-input label="备注" v-model="fromModalData.data.remark" type="textarea"
                                         required></tt-simple-input>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        <button @click="fromModalData.submit" class="btn btn-sm btn-primary pull-right m-t-n-xs"
                                type="button"><strong>确认</strong></button>
                        <button data-dismiss="modal" class="btn btn-sm btn-default pull-right m-t-n-xs tt-modal-cancel"
                                type="button"><strong>取消</strong></button>
                    </div>
                </div>
            </form>
        </tt-modal>
        ...
        <tt-modal id="stock-take-modal" title="盘点">
            <form role="form" class="validation">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <button @click="addStockTake" class="btn btn-sm btn-primary pull-right m-t-n-xs" type="button">
