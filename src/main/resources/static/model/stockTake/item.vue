                    ...
                    <div class="ibox tt-from-table">
                        <div class="ibox-content">
                            <form role="form" class="form-inline validation">
                                ...
                                <tt-simple-input label="任务id" v-model="conditions.stockTakeId"
                                ...
                                <tt-simple-tree-root-v2 label="类型" v-model="conditions.assetsTypeId" :data="tree.assetType"
                                                        :option="{key:'id',value:'name'}"></tt-simple-tree-root-v2>
                                <tt-simple-tree-root-v2 label="网点" v-model="conditions.pointId" :data="tree.point"
                                                        :option="{key:'id',value:'name'}"></tt-simple-tree-root-v2>
                                <!--<tt-simple-select label="网点" v-model="conditions.pointId" :data="Map.point"-->
                                                  <!--show-undefined></tt-simple-select>-->
                                <tt-simple-select label="状态" v-model="conditions.status" :data="Map.stockTakeItemStatus"
                                                  show-undefined></tt-simple-select>
                            ...
                            <div class="table-responsive">
                                <tt-table v-bind:data="tableData" :selection="true" v-model="tableSelectData">
                                    <template slot="tt-body-operation" scope="props">
                                        <button @click="updateToAbnormal(props.row)"
                                                v-shiro:permission="'stockTake:updateToAbnormal'"
                                                v-if="props.row.status === 1"
                                                class="btn btn-table btn-danger btn-rounded" type="button">标记异常
                                        </button>
                                    </template>
                                </tt-table>
                            </div>
